...
OV_ST={'GetEmpirePlanets':'Comms','GetBounties':'Science','ClaimBounty':'Science','CompleteBounty':'Science','GetSystemAnomalies':'Science','ScanAnomaly':'Science','GetPlanet':'Science','ImproveResearch':'Science','SpeedupResearch':'Science','CheckResearchQueue':'Science','CancelQueuedResearch':'Science','GetSystemAsteroids':'Helm','RecruitTroop':'Tactical','BuildDefenseUnit':'Tactical','LoadTroops':'Tactical','UnloadTroops':'Tactical','FleetAttackPlanet':'Tactical','UpdateFleetDefendPosition':'Tactical','Colonize':'Ops','ExplorePlanet':'Science','HarvestAsteroid':'Engineering','DepositCargo':'Ops','WithdrawCargo':'Ops','GetDailyObjectives':'Conseil','GetShopData':'Conseil','GetActivity':'Comms','GetMeEmpire':'Système (boot)','GetEmpires':'Comms','GetRelation':'Comms','GetEmpire':'Comms','GetSystems':'Holo table','changesystem':'Pont — TP de vue','GetLatestNews':'Sas (Menu)','GetGameAnnouncements':'Sas (Menu)','GetEventData':'Conseil','SpeedupFleetTravel':'Helm','AddShip':'Engineering','CheckShipQueue':'Engineering','CancelQueuedShip':'Engineering','SpeedupShipyard':'Engineering','GetResource':'Ops (poll global)','GetConfigs':'Système (boot)','GetTranslations':'Système (boot)'}
OV_PH={'AddFleetOrderStep':'Étape JSON : `targetId` (planète / astéroïde) ; `moveToSystem` avec **`x`,`y`** (le serveur ignore `targetX/targetY`)','RemoveFleetOrderStep':'`stepIndex` 0-based, repeater Helm','ToggleFleetQueueLoop':'`loop` explicite 0/1','MakeBattle':'fleets = mon vaisseau + cibles, `planetid` seulement si ≠ 0 (0 vs pirates), puis `UpdateBattle` (web startTacticalBattle)','Colonize':'`ship` = id du module `ColonyShip` ; planète libre, habitabilité ≥ 6','MoveFleetToSystem':'P4','MoveFleetToPlanet':'P4','MoveFleetToAsteroid':'P4','PrlBondFleetToSystem':'P4','SetFleetOrderQueue':'P4','AddFleetOrderStep':'P4','RemoveFleetOrderStep':'P4','ClearFleetOrderQueue':'P4','ToggleFleetQueueLoop':'P4','GetAllFleets':'P0','GetResource':'P0','GetMeEmpire':'P0','GetConfigs':'P0','GetTranslations':'P0','GetSystems':'P4','BattleDoAction':'P0','GetActivity':'P6','GetLatestNews':'P6','GetGameAnnouncements':'P6','GetEmpires':'P5','GetRelation':'P5','GetEmpire':'P5','GetMyWars':'P5','GetMyAlliance':'P5','GetEmpirePlanets':'P5'}
NOTES={'BattleDoAction':'Envoyer `subaction` + `battle_subaction` (jamais `action`) ; `skill_id=0` pour un move (le serveur refuse un param vide)','GetEmpirePlanets':'Pas de `systemid` dans la réponse : le TP liste mes planètes via `GetSystems` (comme le web)','GetAllFleets':'Cache serveur 3 s → poll VR 3,5 s. Traite les files de flotte.','GetResource':'`EconomyService` : `raw=1` sur **toutes** les planètes (csv ≤ 50) toutes les 10 s — accumule la production et fait avancer les files. La ligne `user` (identifiants) est jetée à la lecture','GetAllFleetsAround':'Interdit en VR (règle AGENTS) — `GetAllFleets` + filtre client','ProcessFleetOrderQueue':'Géré par cron + `GetAllFleets` ; pas d\'UI','DoTurnBattle':'Legacy, non utilisé par le web','GetBattle':'Legacy','UpgradeBuilding':'Console Ops : devis serveur (coût × niveau cible, temps × computer), « Ajouter à la file » si chantier actif ; réponse texte (niveau) ou JSON `queued`','CancelQueuedBuilding':'Param **`id`** (le web envoie `queue_id` → échec) ; réponse JSON `{ok:false,error:clé}` sans préfixe `error:`','AnswerPlanetDecision':'`decision` = **`decision_key`** (chaîne), pas l\'id ; `choice` yes/no','GetPlanetDecisions':'10 décisions / planète / heure, `nextRefreshAt` → compte à rebours','SpeedupBuilding':'Coût Nova affiché avant (gratuit ≤ 60 s, sinon max(10, ⌈6·min^0.82⌉))','DowngradeBuilding':'Immédiat, sans remboursement : confirmation en deux temps','RenamePlanet':'Aucune validation serveur : VR limite à 32 caractères','ImproveResearch':'Clé réelle `combustionDrive` — démo retirée en P1c','RecruitTroop':'VR : Infantry×1 codé en dur','BuildDefenseUnit':'VR : MissileTurret×1 codé en dur','AddShip':'VR : ShipCore codé en dur','AddFleetOrderStep':'Étape JSON : `targetId` (planète / astéroïde) ; `moveToSystem` avec **`x`,`y`** (le serveur ignore `targetX/targetY`)','RemoveFleetOrderStep':'`stepIndex` 0-based, repeater Helm','ToggleFleetQueueLoop':'`loop` explicite 0/1','MakeBattle':'fleets = mon vaisseau + cibles, `planetid` seulement si ≠ 0 (0 vs pirates), puis `UpdateBattle` (web startTacticalBattle)','Colonize':'`ship` = id du module `ColonyShip` ; planète libre, habitabilité ≥ 6','MoveFleetToSystem':'Toujours `hyperspace` explicite (0 sous-lumière / 1 hyperespace) après devis au pupitre (`TravelPlanner`, formules serveur) ; `ok:sublight_*` → `ApiResult.NoticeKey`','PrlBondFleetToSystem':'Devis portée / coût / recharge au pupitre (`TravelPlanner`, distance sur `visual_x/visual_y` comme le serveur), `fleet` + `system` + `pos` ; lâcher sur une étoile de la galaxie','GetSystems':'Galaxie complète sur la table (LOD, territoires par détenteur)','MoveFleetToPlanet':'idem hyperspace','MoveFleetToAsteroid':'idem hyperspace','GetChat':'VR affiche du JSON brut','GetMails':'VR affiche du JSON brut','LoginToken':'Token minté sur le casque, lié à l\'IP','GetDailyObjectives':'Web utilise `GetProgressionObjectives`'}
...
for name,v in sorted(A.items()):
    ...
    if vr and name in ('ImproveResearch','RecruitTroop','BuildDefenseUnit','AddShip','GetChat','GetMails','PlaceShipModule','GetShopData','GetDailyObjectives','GetKnownAddresses','GetJumpgateDestinations','CheckShipQueue','GetMyWars'): status='Démo'
