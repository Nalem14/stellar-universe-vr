...
| **Engineering** | ✅ Cale sèche (Editor) : salle de contrôle + baie, vaisseau à 1:1 dans son berceau reconstruit à chaque pose (étincelles de soudure), table d'assemblage 9×9, caisse de module à attraper et poser, retrait, nouveau vaisseau depuis un ShipCore, destruction de module, renommage. Reste : console de chantier dans la cale (`AddShip` catalogue, `SpeedupShipyard`, file + `CancelQueuedShip`) |
