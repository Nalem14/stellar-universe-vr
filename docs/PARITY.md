...
- **Pas de contrôle de propriétaire sur `CheckBuildingQueue` / `CheckShipQueue` / `RefreshStats`** : ⏸️ **choix assumé, pas un oubli**. Ces trois actions ne renvoient que des données déjà servies publiquement par `GetPlanet` (`SELECT *` sur `planets` — public par conception, cf. plus haut : `working`, `workingtype`, `shipQueue`, `mineralMine`), et le web les interroge délibérément sur une planète **étrangère** où l'on a un vaisseau : `view/game.php` ligne 92 inclut `modal-planet.php` dès que `$planet['userid'] != $_SESSION['id'] && $imHere`, et cette modale poll les trois toutes les 10 s. Un contrôle strict casserait cette vue sans rien protéger. Ce qui fuyait réellement — les bonus de l'appelant — est corrigé ci-dessus. À rouvrir seulement si l'on décide que le rendement d'une planète visitée ne doit pas être visible.
