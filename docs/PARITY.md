...
### Déplacements (corrigé côté web, non commité, 2026-09-26)
...
- **Polices** : côté web, `<html lang="…">` fait choisir au navigateur les bonnes variantes (Han unifié en ja/zh). Côté VR, `Core.Vfx.TmpFonts` accroche un repli CJK dès qu'une langue `cjk` est active. **Asset à fournir** : `Resources/Fonts/CJK SDF` (asset de police TMP, atlas **dynamique**, généré depuis Noto Sans CJK / Source Han Sans). Sans lui ko/ja/zh s'affichent en carrés manquants ; les autres langues ne sont pas concernées (LiberationSans couvre latin + cyrillique).
- **Traductions** : les huit nouveaux `assets/langs/*.json` partent des valeurs **anglaises** — l'outil admin Translate (`admin/panel/.../translate`) les liste et les édite clé par clé. Tant qu'une clé n'est pas traduite, le serveur comme les clients servent l'anglais. Suivi dans [`docs/i18n/missing-keys.md`](i18n/missing-keys.md).
- **Reste à faire** : traduire les huit langues ; `hreflang` de `include/header.php` toujours limité à fr/en (les huit autres n'ont pas d'URL propre) ; les vingt-deux libellés FR/EN en dur de `AnnouncementsModalUI.js` restent à passer en clés.
