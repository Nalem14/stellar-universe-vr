...
### Files, stats, combats (corrigés, 2026-09-26)

- **`DelToFleet` contournait les gardes de `RemoveShipModule`** : ✅ la règle est désormais **partagée** dans `model/ship.php` (`DetachModuleFromFleet`, `HullStaysConnectedWithout`, `HullCellsConnected`), appelée par `RemoveShipModule` (cale sèche) **et** `DelToFleet` (modale PHP legacy) : module à nous, amarré à une flotte, jamais le `ShipCore`, et refus si le retrait couperait la coque du cœur (parcours en largeur depuis le cœur sur les cases restantes). La case de grille est libérée dans les deux cas (`grid_x`/`grid_y` → NULL — `DelToFleet` laissait un module fantôme sur la grille). Logique vérifiée par exécution : 7 cas de connectivité (pont coupé, îlot séparé, coque legacy sans cœur) + 5 cas de flux (introuvable, cœur, pont, succès, module hors grille).
- **`RefreshStats` ignorait les bonus** : ✅ même formule que `GetResource.earn` — main-d'œuvre (`JOBSPERLEVEL`), moral (chômage), booster `production_multiplier`, traits d'espèce `mine` / `food` — et l'énergie compte maintenant les **trois** types de centrale (`orbitSolarPlant`, `solarPlant`, `nuclearPlant`) avec le trait `power` (l'ancien code ne comptait que `solarPlant`). Vérifié par exécution : la valeur est **identique quel que soit le demandeur** et égale à la formule de `GetResource` (unité inchangée : par seconde, les clients multiplient par 86400). Deux écarts restants du bloc `earn` ont été repris au passage : le **niveau du bâtiment en chantier** est déduit (`ProcessPlanetBuildingQueue` l'incrémente au démarrage, `GetResource` le retranche pendant le calcul) et le **déficit d'énergie** effondre la production (`0.001 / 0.0005 / 0.00025`) comme `GetResource` — sans quoi le rapport promettait un rendement que le serveur ne verse pas. 7 cas vérifiés par exécution (dont niveau en chantier déduit et bascule en déficit).
- **L'empire demandeur prêtait ses bonus à la planète** : ✅ `RefreshStats` résout l'empire du **propriétaire** (`GetEmpireByUser($planet['userid'])`), plus celui de l'appelant — c'était la vraie faute derrière le point précédent.
- **`CheckBuildingQueue` / `CheckShipQueue` / `RefreshStats` sans contrôle de propriétaire** : ✅ les trois refusent maintenant une planète qui n'est pas la nôtre (`error_planet_not_yours`, même clé que `SpeedupBuilding`). Les appelants ont été vérifiés côté clients : le client **Phaser** ne construit les onglets de la fenêtre planète (dont Stats → `RefreshStats`) que dans la branche `planet.userid == User.id` — une planète étrangère n'affiche que « déjà colonisée » — et il n'appelle ni `CheckBuildingQueue` ni `CheckShipQueue` ; la VR ne travaille que sur ses propres mondes (`OpsConsole` et `ShipyardPanel` tirent leur planète de `OwnedPlanets`, `ServerTimers` suit la même). Les vues PHP legacy (`view/game.php`, `include/parts/modal-planet.php`) qui les interrogeaient sur une planète visitée ne sont plus servies. Gardes vérifiées par exécution : planète à nous → même JSON qu'avant, planète étrangère → refus.

### Déplacements (corrigé côté web, eb2a11a, 2026-09-26)

- **Le booster `move_speed` ralentissait les trajets** : les Pass Nova le définissent comme un **facteur de durée** (`0.5`, « plus petit = plus rapide », `ActivateBooster` garde le minimum comme pour `explore_speed` / `mining_speed` / `research_speed`, qui multiplient bien des durées), mais les calculs de vitesse de `actionjs.php` (`MoveFleetToSystem`, `MoveFleetToPlanet` ×2, `MoveFleetToAsteroid` ×2) et des cinq étapes de trajet de `model/fleet_queue.php` (file d'ordres) le **multipliaient à la vitesse**. Avec le pass actif, un trajet sous-lumière passait de 5 à 2,5 → `(int)` 2 de vitesse : **2,5× plus long** au lieu de 2× plus court (constaté en réel : 300 s au lieu de 60 s pour (5, 1) → (5, 3)). ✅ `$speed /= GetBoosterMultiplier(…, 'move_speed')` aux dix endroits. La VR applique le même facteur à ses ETA (`Boosters`, lu dans `GetShopData.activeBoosters`) ; le devis web (`getHyperspaceQuoteTo`) l'ignore toujours.

### Langues (livré, 2026-09-26)

Dix langues : `fr`, `en`, `de`, `es`, `it`, `pt`, `ru`, `ko`, `ja`, `zh`.

- **Registre unique** `include/languages.php` (code, nom natif, label, `html lang`, indice de police) : il remplit les sélecteurs, `GetConfigs.languages` et la validation serveur. Ajouter une langue = une ligne ici + `assets/langs/<code>.json`.
- **Serveur** : `DetectLang()` lit `?lang=` → session → cookie → `Accept-Language` (ordre de qualité) et **ignore** un code inconnu au lieu de forcer l'anglais ; `Lang()` / `LangIn()` retombent sur l'anglais quand la clé — ou le fichier — manque ; `GetTranslations` sert la langue de la requête **plus** l'anglais (deux tables, payload inchangé : ~4 600 clés). Les `include/langs/*.php`, morts, ne sont plus requis.
- **Sélecteurs** : le `<select>` de `include/menu.php` (rendu sur la page de jeu) et celui du launcher listent le registre avec les **noms natifs** ; `view/play.php` injecte `window.languages` + `window.lang` ; le client Phaser charge `langs/<langue>.json` **et** `langs/en.json` puis fusionne (l'anglais comble les trous, un fichier manquant ne vide plus l'UI) ; `Helper.getCountryCode()` ne se limite plus à fr/en. `<html lang>` et `<meta name="language">` suivent la langue de la requête.
- **VR** : `Trans` garde une table par langue (plus de `_en`/`_fr`), `Lang` est validé contre le registre, `TryGet` retombe sur l'anglais, et changer de langue vide le dump pour le recharger (`ActionJs` ajoute déjà `&lang=`). Sélecteur dans le **sas** : un bouton du Hub qui fait défiler les langues (libellé = nom natif) et relabelle le panneau ; les écrans construits ensuite (pont, stations) suivent. `ScreenKit.Localized` : toute langue non française lit la variante `_en` du texte serveur.
- **Polices** : côté web, `<html lang="…">` fait choisir au navigateur les bonnes variantes (Han unifié en ja/zh). Côté VR, les trois faces CJK sont **livrées** dans `Assets/_Core/Resources/Fonts/` (`NotoSansJP/KR/SC-Regular.otf`, sous-ensembles par langue — c'est ce qui garde les bonnes variantes Han ; un face japonaise rendrait le chinois avec des formes japonaises) et `Core.Vfx.TmpFonts` construit l'asset TMP **au runtime** (atlas dynamique, glyphes rastérisés à la demande) puis l'accroche au repli de la police d'UI : le build embarque l'.otf (4-8 Mo) au lieu d'un atlas pré-cuit, et il n'y a aucun asset à régénérer. Licence **SIL OFL 1.1** (`Resources/Fonts/LICENSE-OFL.txt`). Latin + cyrillique restent sur LiberationSans.
- **Traductions** : les huit langues sont traduites à partir de l'anglais (97-99 % des 2305 clés ; les valeurs restantes identiques à l'anglais sont des identifiants — `Nova`, `XP`, `Hangar`, `Jumpgate` — ou des mots communs aux deux langues). Chaque fichier a été vérifié : même jeu de clés que `en.json`, même ordre, aucune valeur vide, placeholders `{0}`/`{1}` intacts, noms propres préservés. À relire d'un œil avant de communiquer dessus : l'outil admin Translate (`admin/panel/.../translate`) les édite clé par clé, et toute clé non traduite retombe sur l'anglais.
- **Reste à faire** : `hreflang` de `include/header.php` toujours limité à fr/en (les huit autres n'ont pas d'URL propre) ; les vingt-deux libellés FR/EN en dur de `AnnouncementsModalUI.js` restent à passer en clés ; relire les traductions (surtout les abréviations de combat `vr.battle.status.*`, ajustées par langue).
